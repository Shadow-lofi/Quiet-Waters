...
// Just the lamb's woolly head — a compact face that fills a small circle, for the
// floating helper button. Same warm, theme-stable colors as the full lamb.
export function LambFace({ size = 40, className = '' }: { size?: number; className?: string }) {
  const WOOL = '#f6f9fa'
  const WOOL_EDGE = '#cddfe5'
  const FACE = '#5b7a84'
  const FACE_EDGE = '#4a6b76'
  ...
  // A woolly ring framing the face.
  ...
  return (
    <svg
      ...
      aria-label="A little lamb"
    >
      {/* ears, peeking from under the wool */}
      <g fill={FACE}>
        <ellipse cx="10" cy="32" rx="6" ry="4" transform="rotate(-25 10 32)" />
        <ellipse cx="54" cy="32" rx="6" ry="4" transform="rotate(25 54 32)" />
      </g>

      {/* fleece ring — grey edge, then wool */}
      <g fill={WOOL_EDGE}>
        {ring.map(([cx, cy, r], i) => (
          <circle key={`re-${i}`} cx={cx} cy={cy} r={r + 2.3} />
        ))}
      </g>
      <g fill={WOOL}>
        {ring.map(([cx, cy, r], i) => (
      ...
      {/* face */}
      <ellipse cx="32" cy="35" rx="13" ry="12.5" fill={FACE} />

      {/* eyes */}
      <circle cx="26" cy="35.5" r="2.6" fill={INK} />
      <circle cx="38" cy="35.5" r="2.6" fill={INK} />
      <circle cx="25.2" cy="34.6" r="0.9" fill={WOOL} opacity="0.85" />
      <circle cx="37.2" cy="34.6" r="0.9" fill={WOOL} opacity="0.85" />

      {/* cheeks */}
      <ellipse cx="21" cy="41.5" rx="3.4" ry="2.2" fill="#e7a7a0" opacity="0.5" />
      <ellipse cx="43" cy="41.5" rx="3.4" ry="2.2" fill="#e7a7a0" opacity="0.5" />

      {/* nose + smile */}
      <ellipse cx="32" cy="42" rx="2.2" ry="1.6" fill={FACE_EDGE} />
      <path
        d="M28 45 Q32 48.5 36 45"
        fill="none"
        stroke={INK}
        strokeWidth="1.5"
        strokeLinecap="round"
